...
Module ZipFileCreator
    ...
    Public Function ViewAttachmentPackage_Database(ByVal refno As String)
        Dim extract_location As String = System.IO.Path.GetTempPath() & "Dockeeper\" & getGlobalTrnid("dir", globaluserid) & "\"
        ...
        Try
            ...
            com.CommandText = "select * from tblattachmentlogs where refnumber='" & refno & "'" : rst = com.ExecuteReader
            While rst.Read
            ...
            For Each word In filetoView.Split(New Char() {","c})
                If word <> "" Then
                    com.CommandText = "SELECT * FROM `" & word & "` where refnumber = '" & refno & "'"
    ...
    Public Function AddAttachmentPackage(ByVal refno As String, ByVal trntype As String, ByVal fileLocation As String) As Boolean
        Dim FileSize As UInt32
        Dim rawData() As Byte
        Dim fs As FileStream
        ...
        Try
            If fileLocation <> "" Then
                ...
                fs.Close()
                Dim ArchievedName As String = NewMd5FileName(fileLocation) & Path.GetExtension(fileLocation)
                com.CommandText = "insert into tblattachmentlogs set refnumber='" & refno & "', databasename='tbl" & GetAttachmentName() & "',filename='" & rchar(Path.GetFileName(fileLocation)) & "',archievedname='" & ArchievedName & "', filesize='" & FileSize & "'" : com.ExecuteNonQuery()
                com.CommandText = "INSERT INTO `tbl" & GetAttachmentName() & "` (refnumber,trntype,filename,archievedname,extension,attachment,filesize,datesaved) " _
                    + " VALUES('" & refno & "','" & trntype & "','" & rchar(Path.GetFileName(fileLocation)) & "','" & ArchievedName & "','" & Path.GetExtension(fileLocation) & "',?File,?FileSize,current_timestamp)"
                com.Parameters.Add("?File", rawData)
                com.Parameters.Add("?FileSize", FileSize)
                com.ExecuteNonQuery()

                'add copy to archived
                AddFileToZip(GetAttachmentName(), fileLocation, refno, ArchievedName)
